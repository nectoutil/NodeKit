---
'@necto/constants': major
---

**Breaking change:** The `NUMERIC` export has been removed. It duplicated `numbers.json`'s content (just `DIGITS` in a different order). Use `NUMBERS.DIGITS` instead, which additionally exposes `NUMBERS.PADDED_DIGITS`.
