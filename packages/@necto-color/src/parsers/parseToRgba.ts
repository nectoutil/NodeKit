...
function parseToRgba(color: string): Array<number> {
  ...
  const reducedHexMatch = new RegExp(/^#([a-f0-9])([a-f0-9])([a-f0-9])$/i).exec(normalizedColor);
  if (reducedHexMatch) {
    return [
  ...
  const rgbaMatch = new RegExp(`^rgba?\\(\\s*(\\d+)\\s*,\\s*(\\d+)\\s*,\\s*(\\d+)\\s*(?:,\\s*([\\d.]+))?\\s*\\)$`, 'i').exec(normalizedColor);
  if (rgbaMatch) {
    const [_, r, g, b, a = '1'] = rgbaMatch;
    return [parseInt(r, 10), parseInt(g, 10), parseInt(b, 10), parseFloat(a)];
  }

  const hslaMatch = new RegExp(/^hsla?\(\s*([\d.]+)\s*,\s*([\d.]+)%\s*,\s*([\d.]+)%\s*(?:,\s*([\d.]+(?:\.\d+)?))?\s*\)$/i).exec(normalizedColor);
  if (hslaMatch) {
    const [h, s, l, a] = hslaMatch.slice(1).map(parseFloat);
    if (Math.min(Math.max(0, s), 100) !== s) throw new Error(`[__PACKAGE_NAME__]: Invalid saturation value: ${s}`);
    if (Math.min(Math.max(0, l), 100) !== l) throw new Error(`[__PACKAGE_NAME__]: Invalid lightness value: ${l}`);
