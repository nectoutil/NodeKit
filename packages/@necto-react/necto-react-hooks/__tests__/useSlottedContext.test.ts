...
describe('useSlottedContext', () => {
  it('returns the direct context value when no slots are present', () => {
    const Context = createContext<
      { label: string } | { slots?: Record<string | symbol, { label: string }> } | null | undefined
    >({ label: 'direct' });

    const { result } = renderHook(() => useSlottedContext({ context: Context }));
  ...
  it('returns null when slot is explicitly null', () => {
    ...
    const { result } = renderHook(() => useSlottedContext({ context: Context, slot: null }));
  ...
  it('returns the value for a valid slot', () => {
    ...
    const { result } = renderHook(() => useSlottedContext({ context: Context, slot: 'header' }));
  ...
  it('throws an error for an invalid slot', () => {
    ...
    ).toThrowError(/Invalid slot name: "footer"\. {2}This slot is not recognized/);
  });
  ...
  it('throws if slots exist but are empty', () => {
    ...
    ).toThrowError(/Invalid slot name: "header"\. {2}This slot is not recognized/);
  });
});
